...
from QA_model.Bert.tokenization import BertTokenizer
...
class BatchGen_QuAC:
    def __init__(self, data, batch_size, gpu, dialog_ctx=0, use_dialog_act=False, evaluation=False, context_maxlen=100000, precompute_elmo=0, use_bert = None, bert_path = None):
        '''
        input:
            data - see train.py
            batch_size - int
        ...
        if self.use_bert == 'large':
            tokenizer_file = os.path.join(bert_path, 'bert-large-uncased-vocab.txt')
            self.bert_tokenizer = BertTokenizer.from_pretrained(tokenizer_file)
        elif self.use_bert is None:
            self.tokenizer_file = None
        else:
            tokenizer_file = os.path.join(bert_path, 'bert-base-uncased-vocab.txt')
            self.bert_tokenizer = BertTokenizer.from_pretrained(tokenizer_file)
    ...
    def __iter__(self):
        ...
        for batch_i in range((self.context_num + self.batch_size - 1) // self.batch_size):
            ...
            yield (context_id, context_cid, context_feature, context_tag, context_ent, context_mask,
                   ...
                   text, span, question, answer,
                   context_bert, context_bert_mask, context_bert_offsets,
