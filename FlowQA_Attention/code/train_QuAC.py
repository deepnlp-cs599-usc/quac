...
parser.add_argument('--flow_attention', type=int, default = 1)
parser.add_argument('--use_bert', help="Whether to use BERT or not, and which version to use")
parser.add_argument('--bert_path', default='bert_vocab_files/')
...
def main():
    ...
    if args.resume:
        batches = BatchGen_QuAC(dev, batch_size=args.batch_size, evaluation=True, gpu=args.cuda, dialog_ctx=args.explicit_dialog_ctx, use_dialog_act=args.use_dialog_act, use_bert=args.use_bert, bert_path=args.bert_path)
    ...
    for epoch in range(epoch_0, epoch_0 + args.epoches):
        ...
        # train
        batches = BatchGen_QuAC(train, batch_size=args.batch_size, gpu=args.cuda, dialog_ctx=args.explicit_dialog_ctx, use_dialog_act=args.use_dialog_act, precompute_elmo=args.elmo_batch_size // args.batch_size, use_bert = args.use_bert, bert_path=args.bert_path)
        start = datetime.now()
        ...
        if epoch % args.eval_per_epoch == 0:
            batches = BatchGen_QuAC(dev, batch_size=args.batch_size, evaluation=True, gpu=args.cuda, dialog_ctx=args.explicit_dialog_ctx, use_dialog_act=args.use_dialog_act, precompute_elmo=args.elmo_batch_size // args.batch_size, use_bert = args.use_bert, bert_path=args.bert_path)
            predictions, no_ans_scores = [], []
            for batch in batches:
                phrases, noans = model.predict(batch)
                predictions.extend(phrases)
