...
class FlowQA(nn.Module):
    ...
    def __init__(self, opt, embedding=None, padding_idx=0):
        ...
        if opt['use_bert']:
            print('BERT is being used.')
            self.Bert = Bert(opt) #?

            if opt['use_bert'] == 'large':
                print('BERT large is being used.')
                bert_dim = 1024
                bert_layers = 24
            else:
                print('BERT base is being used.')
                bert_dim = 768
                bert_layers = 12

            print('BERT dim: ', bert_dim, ' BERT layers: ', bert_layers)
            doc_input_size += bert_dim
            que_input_size += bert_dim
        ...
        if opt["flow_attention"]:
            print('Flow attention is being used.')
        ...
        if opt['flow_attention']:
            self.flow_attn1 = layers.HistoryAttention(opt['hidden_size'], opt['deep_att_hidden_size_per_abstr'])
            self.flow_attn2 = layers.HistoryAttention(opt['hidden_size'], opt['deep_att_hidden_size_per_abstr'])
            self.flow_attn3 = layers.HistoryAttention(opt['hidden_size'], opt['deep_att_hidden_size_per_abstr'])
        else:
            self.flow_attn1 = None
            self.flow_attn2 = None
            self.flow_attn3 = None


        # Store config
        self.opt = opt

    def forward(self, x1, x1_c, x1_f, x1_pos, x1_ner, x1_mask, x2_full, x2_c, x2_full_mask, x_bert, x_bert_mask, x_bert_offsets, q_bert, q_bert_mask, q_bert_offsets):
        """Inputs:
        ...
        if self.opt['use_bert']:
            x_cemb_mid = self.Bert(x_bert, x_bert_mask, x_bert_offsets, x1_mask)
            x_cemb_mid = x_cemb_mid.expand(x2_full.size(0), -1, -1)
            ques_cemb_mid = self.Bert(q_bert, q_bert_mask, q_bert_offsets, x2_full_mask)

            drnn_input_list.append(x_cemb_mid)
            qrnn_input_list.append(ques_cemb_mid)
        ...
        def flow_operation(cur_h, flow, attn = None):
            ...
            if self.opt['flow_attention']:
                attentive_flow_out = attn(flow_out)
                flow_out = torch.cat((flow_out, attentive_flow_out), 2)
        ...
        doc_hiddens_flow = flow_operation(doc_hiddens, self.dialog_flow1, attn = self.flow_attn1)
        ...
        doc_hiddens_flow = flow_operation(doc_hiddens, self.dialog_flow2, attn = self.flow_attn2)
        doc_abstr_ls.append(doc_hiddens)
        ...
        doc_hiddens_flow = flow_operation(doc_hiddens, self.dialog_flow3, attn = self.flow_attn3)
